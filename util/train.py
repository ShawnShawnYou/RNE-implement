...
def training_hier(model, alpha_list, sample_set):
    for sample in sample_set:
        ...
        derivative_s = 2 * (vector_s - vector_t -
                            shortest_path_value / 10**get_config("norm_factor") * np.sign(vector_s - vector_t))

        # derivative_t = 2 * (shortest_path_value / 10**10 * np.sign(vector_s - vector_t)
        #                     - vector_s)
